...
```{r}
bookings_df <- read_csv("~/Git/r-coursera/importing_data/hotel_bookings.csv")
```
...
example_df <- bookings_df %>%
  ...
  select(adults, children, babies, guests)
...
example_df <- bookings_df %>%
  group_by(hotel_type = hotel) %>% 
  drop_na() %>% 
  summarize(number_canceled = sum(is_canceled),
            average_lead_time = mean(lead_time))

total_number_canceled <- sum(example_df$number_canceled)

total <- tibble(hotel_type = "TOTAL",
                number_canceled = total_number_canceled,
                average_lead_time = NA)

output_df <- add_row(example_df, !!!total)
